        						...
        						<div class="form-row">
									...
									<div class ="form-group col-sm-5">
										<div class="slidecontainer">
  											<input type="range" min="1" max="10" value="1" class="slider" id="TasteRange" name = "filterTaste">
  										</div>
									</div>
								</div>
        					</div>
 			...
 			<div class ="form-group col-sm-10">
 				<div class="btn-group-toggle" data-toggle="buttons">
 					...
 					<label class="btn btn-warning btn-xs" id="c123">
	 					<input type = "radio" name ="filterType" id = "Anywhere" value = "4">저장
 					</label>
 				</div>
 			</div>
 		</div>
...
<input id="pac-input" class="controls" type="text"
  ...
  <script>
      ...
      function CenterControl(controlDiv, map) {
          ...
          controlText.innerHTML = "<img src='img/filter.png'>"
      ...
      <%
      Object seq = session.getAttribute("SEQ_User");


      String a = seq.toString();
		
      System.out.println(a);
      %>

      var locations = <%=placesDAO.followerpin(a)%>;
 	                 ...
 	                 success:function(data)
 	                 {
 	                	 if(n != temp && state == 'none'){ // 처음
 	                		...
 	                		$('#result_img').attr("src",data.file);
 	                		 ...
 	                		 console.log(data.Name + data.Address + data.SaveType + data.Colleague);
 	                		 $("#resultToggle").show();
 			...
 			$("#CompleteFilter").on('click',function(){
 				...
 				var allData = { "Menu": FMenu, "Taste": FTaste, "Price1" : FPrice1, "Price2" : FPrice2, "FoodList" : FFoodList, "Colleague" : FColleagueList, "Follow" : filterFollower, "SaveList" : FSaveList};
 		          $.ajax({
 		             ...
 		             {
 		            	var arr1 = data.a;
 		            	/* var array = new Array(new Array(arr1.length), new Array(3)); */
 		            	var array = [];
 		            	for(var x = 0 ; x < 24; x ++){
 		            		array[x] = [];
 		            	}
